...
func (web *Web) rankingsPdfReportHandler(w http.ResponseWriter, r *http.Request) {
	...
	colWidths := map[string]float64{"Rank": 15, "Team": 25, "RP": 25, "Match": 25, "Park": 25,
		"W-L-T": 30, "DQ": 25, "Played": 25}
	rowHeight := 6.5
	...
	pdf.CellFormat(colWidths["Park"], rowHeight, "Park", "1", 0, "C", true, 0, "")
	...
	for _, ranking := range rankings {
		...
		pdf.CellFormat(colWidths["Park"], rowHeight, strconv.Itoa(ranking.ParkPoints), "1", 0, "C", false, 0, "")
...
func (web *Web) schedulePdfReportHandler(w http.ResponseWriter, r *http.Request) {
	...
	// The widths of the table columns in mm, stored here so that they can be referenced for each row.
	colWidths := map[string]float64{"Time": 35, "Match": 40, "Team": 60}
	rowHeight := 6.5
